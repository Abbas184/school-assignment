...
export default function ProfilePage() {
  ...
  // This initializes two separate form instances
  ...
  const onUpdateDetails = async (data) => {
    ...
    try {
      const res = await fetch('/api/profile', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name: data.name, mobile: data.mobile }) });
  ...
  const onUpdatePassword = async (data) => {
    ...
    try {
      const res = await fetch('/api/profile/change-password', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ currentPassword: data.currentPassword, newPassword: data.newPassword }) });
      const result = await res.json();
      if (res.ok) {
      ...
      <main className="flex-grow container mx-auto p-4 md:p-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            <h2 className="text-xl font-semibold mb-4 text-gray-800">Update Your Details</h2>
            <form onSubmit={handleSubmitDetails(onUpdateDetails)} className="space-y-4">
              ...
              <div>
                ...
                <input type="email" {...registerDetails('email')} disabled className="w-full p-2 border rounded-md bg-gray-100" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-800">Mobile Number</label>
                <input type="tel" {...registerDetails('mobile', { required: 'Mobile is required' })} className="w-full p-2 border rounded-md" />
                {detailsErrors.mobile && <p className="text-red-500 text-xs mt-1">{detailsErrors.mobile.message}</p>}
              </div>
              <button type="submit" className="w-full bg-blue-600 text-white py-2 rounded-md">Save Changes</button>
            </form>
          </div>
          ...
          <div className="bg-white p-6 rounded-lg shadow-md">
            ...
            <form onSubmit={handleSubmitPassword(onUpdatePassword)} className="space-y-4">
              ...
              <button type="submit" className="w-full bg-gray-700 text-white py-2 rounded-md">Change Password</button>
            </form>
          </div>
        </div>
      </main>
